...
export function promptMarkup(key,open=false){
  ...
  return `<details class="prompt-card composer-card" ${open?'open':''} id="opdracht-${p.key}"><summary><span class="prompt-id">${p.key.startsWith('s')?'SEO':String(p.id).padStart(2,'0')}</span><span class="composer-title">${esc(p.title)}<small>${esc(c.when||'Gebruik de uitkomst van de vorige stap en voeg je eigen informatie toe.')}</small></span></summary><div class="prompt-content"><div class="composer-tool"><div><span class="eyebrow">1 · Hier plak je de opdracht</span><div class="tool-choice" role="group" aria-label="AI-tool voor ${esc(p.title)}">${c.tools.length>1?c.tools.map(t=>`<button data-prompt-tool="${p.key}" data-tool="${t}" aria-pressed="${selected===t}">${esc(tools[t].name)}</button>`).join(''):`<strong>${tools[selected].name}</strong>`}</div></div><a data-tool-open="${p.key}" href="${tools[selected].url}" target="_blank" rel="noopener noreferrer">Open ${tools[selected].name} ↗</a></div><div class="composer-input"><span class="eyebrow">2 · Geef jouw input mee</span>${c.needsGoal?'<p class="project-reminder">Je projectidee bovenaan gaat automatisch mee.</p>':''}${c.fields.map(f=>fieldMarkup(p.key,f)).join('')}</div><div class="composer-result"><span class="eyebrow">Dit wil je terugkrijgen</span><p data-prompt-result="${p.key}">${esc(expectedResult(p.key))}</p></div><div class="composer-copy"><div><button class="copy-btn" data-composer-copy="${p.key}" ${copyId}>Kopieer mijn opdracht</button><small data-ready="${p.key}" ${missing.length?'':'class="ready"'}>${missing.length?`Vul nog ${missing.length} ${missing.length===1?'veld':'velden'} in.`:'Klaar om te kopiëren, inclusief jouw input.'}</small></div><p>Plak de opdracht in <span data-tool-name="${p.key}">${tools[selected].name}</span>. Voeg daar zelf je bestanden toe en verstuur hem.</p></div><details class="prompt-preview"><summary>Bekijk de volledige opdracht</summary><textarea class="prompt-text" data-preview="${p.key}" id="prompt-text-${p.key}" rows="10" readonly spellcheck="false" aria-label="Volledige opdracht: ${esc(p.title)}">${esc(composePrompt(p.key))}</textarea><p>De werkwijze, je projectcontext en je ingevulde antwoorden worden samen gekopieerd.</p></details><p class="composer-home"><a href="${p.href}">${p.home} ↗</a></p></div></details>`;
}
export function lessonPromptMarkup(id){
  const s=stepPrompts[id];
  return `<h2 class="panel-heading">Van jouw input naar een goede opdracht.</h2><p class="prompt-intro">Vul je project in, voeg de informatie voor deze stap toe en kopieer de complete opdracht. Je hoeft zelf geen prompt te schrijven.</p>${projectMarkup()}${s.main.map((key,i)=>promptMarkup(key,i===0)).join('')}${s.extra.length?`<details class="extra-prompts"><summary>Ook handig in deze stap <span>${s.extra.length} gerichte ${s.extra.length===1?'opdracht':'opdrachten'}</span></summary><p>Kies alleen wat je nu nodig hebt. Deze opdrachten zijn geen verplichte extra stappen.</p>${s.extra.map(key=>promptMarkup(key)).join('')}</details>`:''}`;
}
export function refreshPreviews(){
  ...
  document.querySelectorAll('[data-ready]').forEach(el=>{const count=missingInput(el.dataset.ready).length;el.textContent=count?`Vul nog ${count} ${count===1?'veld':'velden'} in.`:'Klaar om te kopiëren, inclusief jouw input.';el.classList.toggle('ready',!count);});
...
export async function handlePromptClick(event,notify,rerender){
  ...
  try{if(!navigator.clipboard?.writeText)throw new Error('Unavailable');await navigator.clipboard.writeText(text);notify('Je complete opdracht is gekopieerd, inclusief je eigen input. Plak hem in de gekozen AI-tool.');}
