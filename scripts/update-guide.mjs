...
for(const l of seoLessons){
  ...
  lines.push('','**Opdracht voor deze les:**','','```text',l.prompt,'```','','**Officiële uitleg:**','');bullets(l.sources.map(([title,url])=>`[${title}](${url})`));lines.push('');
}
lines.push('## Algemene AI-opdrachten','','Dit zijn de basisopdrachten. De Promptgenerator voegt je projectgegevens, stapinput en werkafspraken toe. Gebruik opdrachten.md voor volledige invulbare opdrachten. Voeg genoemde bestanden ook echt toe in de AI-tool of open je projectmap in Antigravity.','');
for(const p of prompts)lines.push(`### Opdracht ${String(p.id).padStart(2,'0')} — ${p.title}`,'','```text',p.text,'```','',`**Resultaat:** ${p.result}`,'');
