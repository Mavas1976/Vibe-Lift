...
De deelmetadata gebruikt het openbare Railway-adres `https://vibe-lift-production.up.railway.app/` en de daar gehoste `og-vibe-lift-logo-v2.png`. Dit is het actuele headerlogo op wit; `og.png` bevat dezelfde afbeelding voor oude verwijzingen. Verwijs hiervoor niet naar de afgeschermde Sites-publicatie: sociale crawlers kunnen die afbeelding niet zonder inloggen ophalen. Controleer na publicatie met `node scripts/check-social.mjs --live` dat de echte HTML en PNG zonder credentials bereikbaar zijn voor WhatsApp- en Facebook-crawlerheaders. Deze controle simuleert HTTP-verzoeken; hij controleert niet de cache of weergave in de apps zelf.
